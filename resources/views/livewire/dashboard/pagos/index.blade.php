                ...
                <div class="column is-8">
                    <div class="control has-icons-left">
                        <input type="text" class="input" wire:keydown.enter="buscar" wire:model.defer="search"
                            placeholder="Buscar por codigo de matricula" />
        ...
        <div class="content-dashboard-search-bar">
            <div class="columns">
                <div class="column has-text-centered"><i class="fas fa-circle has-text-warning"></i>
                    <strong>{{ $pendientes }}</strong> POR REVISION
                </div>
                ...
                <div class="column has-text-centered"><i class="fas fa-circle has-text-danger"></i>
                    <strong>{{ $anulados }}</strong> ANULADOS
                </div>
                ...
                <tbody>
                    @forelse($pagos as $pago)
                        <tr>
                            ...
                            <td>{{ $pago->matricula->alumno->apellido_paterno }},
                                {{ $pago->matricula->alumno->nombres }}</td>
                            <td>{{ $pago->matricula->nivel == 'P' ? 'PRIMARIA' : 'SECUNDARIA' }}/{{ $pago->matricula->grado | grado }}
                            </td>
                                        ...
                                        @if ($pago->estado == 0)
                                            <div class="items-option" @click.away="open = false">
                                                <a
                                                    ...
                                                    <i class="fas fa-check-double has-text-success"></i> Confirmar pago
                                                    y matrícula</a>
                                            </div>
                                            <div class="items-option" @click.away="open = false">
                                                <a
