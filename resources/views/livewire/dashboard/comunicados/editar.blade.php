    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-300 p-6 md:p-8 space-y-6">
        ...
        <div class="space-y-4">
            ...
            @if (!empty($existingAttachments))
                <div class="p-4 bg-blue-50/50 rounded-lg border border-blue-100">
                    <p class="text-sm font-semibold text-blue-900 mb-3">Archivos adjuntos actuales:</p>
            ...
            @if (!empty($attachments))
                <div class="mt-2 flex items-center gap-2 text-sm text-colegio-600 font-medium">
                    <i class="ph-fill ph-check-circle"></i> {{ count($attachments) }} nuevo(s) archivo(s) validado(s).
                </div>
            @endif
        ...
        <div class="pt-6 border-t border-gray-300 flex flex-col sm:flex-row gap-3 justify-end">
            ...
            <button wire:click="save" type="button"
                ...
                <i class="ph ph-arrows-clockwise mr-2"></i> Actualizar Comunicado
            </button>
        </div>
    </div>
</div>
