    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 font-medium">Perfil y actividad del personal</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('dashboard.profesores') }}"
                class="inline-flex items-center px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-gray-500 hover:bg-gray-50 transition-all shadow-sm">
        ...
        <div class="lg:col-span-4 space-y-6">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center relative overflow-hidden">
                ...
                <div class="relative z-10">
                    ...
                    <p class="text-lg font-bold text-gray-500 uppercase tracking-tight">{{ $teacher->apellidos }}</p>
                    <p class="text-sm font-mono text-gray-400 mt-1 tracking-widest">{{ $teacher->documento }}</p>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                @if ($teacher->user)
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                            <span class="text-sm text-gray-500 font-medium">Usuario</span>
                            <span
                        ...
                        <div class="flex items-center justify-between pt-2">
                            <span class="text-sm text-gray-500 font-medium">Último acceso</span>
                ...
                @else
                    <div class="py-4 text-center">
                        ...
                        <p class="text-sm text-gray-400 italic">No tiene un usuario configurado.</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="lg:col-span-8 flex flex-col space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-1.5 flex gap-1">
                <button wire:click="setTab('info')"
                    class="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all {{ $activeTab === 'info' ? 'bg-colegio-600 text-white shadow-lg shadow-colegio-100' : 'text-gray-500 hover:bg-gray-50' }}">
                    <i class="ph ph-info text-lg"></i> Información
                </button>
                <button wire:click="setTab('appointments')"
                    class="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all {{ $activeTab === 'appointments' ? 'bg-colegio-600 text-white shadow-lg shadow-colegio-100' : 'text-gray-500 hover:bg-gray-50' }}">
                    <i class="ph ph-calendar-check text-lg"></i> Citas
                    <span
                        class="ml-1 px-1.5 py-0.5 rounded-full text-[10px] {{ $activeTab === 'appointments' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500' }}">
                        {{ \App\Models\Appointment::where('teacher_id', $teacher->id)->count() }}
                    </span>
                </button>
                <button wire:click="setTab('messages')"
                    class="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all {{ $activeTab === 'messages' ? 'bg-colegio-600 text-white shadow-lg shadow-colegio-100' : 'text-gray-500 hover:bg-gray-50' }}">
                    <i class="ph ph-envelope-simple text-lg"></i> Mensajes
                    <span
                        class="ml-1 px-1.5 py-0.5 rounded-full text-[10px] {{ $activeTab === 'messages' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500' }}">
                        {{ $teacher->user ? \App\Models\AgendaMessage::where('teacher_user_id', $teacher->user->id)->count() : 0 }}
                    </span>
                </button>
            </div>

            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex-1 min-h-[500px]">
                @if ($activeTab === 'info')
                    <div class="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                            <div class="space-y-1">
                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Nombres y
                                    Apellidos</p>
                                <p class="text-gray-800 font-bold uppercase">{{ $teacher->nombres }}
                                    {{ $teacher->apellidos }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">DNI / Documento
                                </p>
                                <p class="text-gray-800 font-mono font-bold tracking-tighter text-lg">
                                    {{ $teacher->documento }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Correo
                                    Electrónico</p>
                                <p class="text-gray-800 font-semibold">
                                    {{ strtolower($teacher->email ?? 'no registrado') }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Teléfono /
                                    Celular</p>
                                <div class="flex items-center gap-2">
                            ...
                            <div class="space-y-1">
                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Horario Laboral
                                </p>
                                <span
                    ...
                    <div class="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        <div class="bg-gray-50/50 rounded-2xl p-4 border border-gray-100">
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <div class="space-y-1">
                                    <label
                                        class="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-1">Nivel</label>
                                ...
                                <div class="space-y-1">
                                    <label
                                        class="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-1">Grado</label>
                                ...
                                <div class="space-y-1">
                                    <label
                                        class="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-1">Fecha</label>
                                ...
                                <div class="flex items-end text-gray-400">
                                    <button
                                        class="w-full h-10 flex items-center justify-center gap-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-500 hover:bg-gray-100 transition-all shadow-sm"
                        ...
                        <div class="overflow-hidden rounded-2xl border border-gray-100">
                            <table class="w-full text-sm text-left">
                                <thead
                                    class="bg-gray-50 text-gray-500 uppercase text-[10px] font-bold tracking-widest border-b border-gray-100">
                                    <tr>
                                        <th class="px-5 py-3">Fecha y Hora</th>
                                        <th class="px-5 py-3">Alumno</th>
                                        <th class="px-5 py-3">Padre/Madre</th>
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4"
                                                class="px-5 py-12 text-center text-gray-300 italic text-sm">No hay
                        ...
                        <div class="space-y-3">
                            @forelse($messages as $msg)
                                <div wire:click="openAgendaModal({{ $msg->id }})"
                                    class="p-5 bg-white border border-gray-100 rounded-2xl hover:border-colegio-200 hover:shadow-md transition-all cursor-pointer flex items-center justify-between group">
                                    <div class="flex items-center gap-4">
                                        <div
                                            class="w-12 h-12 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-colegio-50 group-hover:text-colegio-600 transition-colors shadow-sm">
                                        ...
                                        <div>
                                            <p
                                                ...
                                                {{ $msg->subject }}</p>
                                            <p
                                                class="text-[10px] text-gray-400 uppercase font-bold tracking-wider mt-0.5">
            ...
            <div
                ...
                <div class="p-8 overflow-y-auto space-y-6">
                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-1">
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Alumno</p>
                            <p class="text-sm font-bold text-gray-800">
                                {{ $selectedAgenda->matricula->alumno->nombre_completo }}</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Asunto</p>
                    ...
                    <div class="space-y-4 pt-4">
                        <div class="flex items-center gap-2">
                            <i class="ph ph-arrow-u-up-left text-gray-400"></i>
                            <h4 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest text-colegio-600">
                            ...
                            @forelse($selectedAgenda->replies as $reply)
                                <div
                                    ...
                                    <div
                                        ...
                                        <div
                                            class="p-4 rounded-3xl text-sm leading-relaxed shadow-sm {{ $reply->author_type === 'parent' ? 'bg-gray-50 text-gray-700 rounded-tl-none border border-gray-100' : 'bg-colegio-600 text-white rounded-tr-none' }}">
                                            {{ $reply->message }}
                                        </div>
                                        <p class="text-[9px] font-bold text-gray-400 mt-2 uppercase tracking-widest">
