<div class="content-dashboard">
    ...
    <div class="columns">
        <div class="column has-text-right">
            <button wire:click="getCarnet" class="button">Generar QR's <i class="ml-2 fas fa-qrcode"></i></button>
                        ...
                        <div class="dashboard-menu-opcion" x-data="{ open: false }">
                            ...
                            <div class="items" x-show="open">
                                <div class="items-option" @click.away="open = false">
                                    @if($teacher->estado == 0)
                                    <a wire:click="activar({{ $teacher->id }})"><i class="fas fa-check-double has-text-success"></i> Confirmar</a>
                                    @elseif($teacher->estado == 1)
                                    <a wire:click="desactivar({{ $teacher->id }})"><i class="fas fa-ban has-text-danger"></i> Anular</a>
                                    @endif
                                </div>
                            </div>
                        </div>
