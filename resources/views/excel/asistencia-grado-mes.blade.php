...
<table>
    <thead>
        ...
        <tr>
            ...
            <th colspan="{{ count($datosReporte['diasLaborables']) }}" style="text-align: left;">
                {{ $datosReporte['gradoInfo']->nivel == 'P' ? 'Primaria' : 'Secundaria' }}</th>
        </tr>
        <tr>
            <th colspan="2" style="font-weight: bold; text-align: left;">Grado:</th>
            <th colspan="{{ count($datosReporte['diasLaborables']) }}" style="text-align: left;">
                {{ strtoupper($datosReporte['gradoInfo']->nombre ?? '') }}</th>
        </tr>
        <tr></tr>
        <tr>
        ...
        <tr>
            ...
            @foreach ($datosReporte['diasSemana'] as $diaInfo)
                <th style="border: 1px solid #000; background-color: #d9d9d9; font-weight: bold; text-align: center;">
                    {{ $diaInfo['letra'] }}</th>
            @endforeach
        </tr>
    </thead>
