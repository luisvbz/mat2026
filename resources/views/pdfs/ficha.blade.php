...
<body>
    ...
    <div class="fecha-matricula" style="margin-bottom: 10px;">
        <table class="tg">
            <thead>
                <tr>
                    <th colspan="3" class="tg-x9uu" style="font-size: 14px;">FICHA DE MATRÍCULA
                        {{ $matricula->anio }}</th>
                </tr>
                <tr>
    ...
    <div class="datos-estudiante" style="margin-bottom: 10px;">
        <table class="tg">
            ...
            <tr>
                ...
                <td class="tg-m5nv">{{ $matricula->alumno->celular }} / {{ $matricula->alumno->telefono_emergencia }}
                </td>
            </tr>
            <tr>
    ...
    @foreach ($matricula->alumno->padres as $padre)
        <div class="datos-padre" style="margin-bottom: 10px;">
            <table class="tg">
                <tr>
                    <th class="tg-x9uu" colspan="4">DATOS
                        {{ $padre->parentesco == 'P' ? 'DEL PADRE' : 'DE LA MADRE' }}</th>
                </tr>
                <tr>
                    <th class="tg-x9uu">Tipo Doc.</th>
    ...
    @foreach ($matricula->alumno->apoderados as $apoderado)
        <div class="datos-padre" style="margin-bottom: 10px;">
            <table class="tg">
                <tr>
                    <th class="tg-x9uu" colspan="4">DATOS DEL APODERADO Y/O RESPONSABLE ECONÓMICO DISTINTO A
                        PADRE/MADRE</th>
                </tr>
                <tr>
                    <th class="tg-x9uu">Tipo Doc.</th>
            ...
            <tr>
                <td>
                    <div style="padding: 3px; border: 1px solid #ccc;">
                        <img src="data:image/png;base64,{!! base64_encode(
                            QrCode::format('png')->margin(0)->size(93)->generate("https://2026.iepdivinosalvador.net.pe/verificador-matricula/$matricula->codigo"),
                        ) !!}" alt="QrCode">
                    </div>
                </td>
                @if (!$matricula->nombres_dj)
                    <td style="font-size: 14px; line-height: 20px;">
                        Yo,
                        <strong>______________________________________________________________________________________</strong>
                        con documento <strong>________________________________________</strong> declaro que soy el
                        responsable del pago de la matricula y pensiones de enseñanza.
                    </td>
                @else
                    <td style="font-size: 14px; line-height: 20px;">
                        Yo,
                        <strong>{{ $matricula->nombres_dj }}</strong> con documento
                        <strong>{{ $matricula->tipo_documento_dj }} {{ $matricula->numero_documento_dj }}</strong>
                        declaro
    ...
    <div style="padding: 20px; margin: 15px;">
        ...
        <p style="text-align: justify; font-size: 17px;">
            Yo, <b>{{ $matricula->salud->nombres }}</b>, con documento de identidad N°
            <b>{{ $matricula->salud->numero_documento }}</b>,
            en representación de mi hijo(a) con nombre
            <b>{{ $matricula->alumno->apellido_paterno . ' ' . $matricula->alumno->apellido_materno }},
                {{ $matricula->alumno->nombres }}</b>
            con documento de identidad N° <b>{{ $matricula->alumno->numero_docuemento }}</b>,
            declaro bajo juramento que, autorizo e informo a la Instituciòn Educativa Privada (En adelante el
            “COLEGIO”),
            con Ruc 10254934475 y domicilio en Jr. Cailloma Nº 574 Cercado de Lima, de lo siguiente:
        </p>
        <ol>
            <li>Conozco, que el COLEGIO no tiene convenio con ninguna clínica, hospital u otros análogos para la
                contratación de los seguros de salud de mi hijo(a), por lo que, no he sido informado de ningún programa
                de protección de salud.</li>
            <li>
                Informo, que soy el único responsable de la protección de salud de mi hijo(a), por lo que, pongo de
                conocimiento del COLEGIO, el seguro de protección de salud y el establecimiento donde deberá ser
                derivado mi hijo(a) en caso de emergencia:<br>
                ...
                <b>Tipo de Seguro: </b>
                {{ $matricula->salud->tipo_seguro != 'OTRO' ? $matricula->salud->tipo_seguro : $matricula->salud->otro_seguro }}.<br>
            </li>
            <li>Acepto, que soy el único responsable en mantener activo el seguro de protección de salud de mi hijo(a),
                por lo que, los gastos derivados en caso de desprotección ante una emergencia son de mi responsabilidad.
            </li>
            <li>Conozco, que en caso de emergencia el COLEGIO activará su protocolo de contingencias administrativas,
                referido al procedimiento de emergencia por accidentes.</li>
        </ol>
        <div style="position: fixed; bottom: -15px;">
            <table style="width: 100%;">
                <tr>
                    <td style="font-size: 12px;">
                        Yo, <strong>{{ $matricula->salud->nombres }}</strong> con documento
                        <strong>{{ $matricula->salud->tipo_documento }}
                            {{ $matricula->salud->numero_documento }}</strong> declaro que soy el unico responsable del
                        seguro de salud de mi hijo(a).
                    </td>
                </tr>
            </table>
        </div>
